...
HANDLE hClientConnectedEvent = CreateEvent(NULL, FALSE, FALSE, "ClientConnected");
...
TalkersCommand set_param(char* param) {
	...
	if (!strcmp(param, "exit"))
		return EXIT;
	if (!strcmp(param, "wait"))
		return WAIT;
	if (!strcmp(param, "shutdown"))
		return SHUTDOWN;
	if (!strcmp(param, "statistics"))
		return STATISTICS;
	if (!strcmp(param, "getcommand"))
		return GETCOMMAND;
	if (strstr(param, "UNLOAD_LIB"))
		return UNLOAD_LIB;
	if (strstr(param, "LOAD_LIB"))
		return LOAD_LIB;
}
	...
	while (*(TalkersCommand*)pPrm != EXIT) {
		try {
			if (GetRequestFromClient((char*)"Hello", serverPort, &from, &lc)) {
				if (inet_ntoa(from.sin_addr) == std::string("192.168.112.189")) {
					printf("Connected another server: %d, port: %d, address: %s\n", ++numberOfClients, htons(from.sin_port), inet_ntoa(from.sin_addr));
				}
				else {
...
int main(int argc, char* argv[]) {
	if (argc == 2) {
		...
		strcpy(dllName, "libServiceLibrary.dll");
		...
		serverPort = 2000;
		strcpy(dllName, "..\\ServiceLibrary\\ServiceLibrary.dll");
		strcpy(namedPipeName, "Tube");
	}
	printf("Server port %d\n", serverPort);
	st = LoadLibraryA(dllName);
	if(st == NULL) {
		cout<< "exception Lib"<<endl;
		return -1;
	}
