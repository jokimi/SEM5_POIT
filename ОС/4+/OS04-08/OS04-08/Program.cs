class Program
{
    ...
    {
        // Запуск потоков через пул потоков
        for (int i = 0; i < ThreadCount; ++i)
        {
            ThreadPool.QueueUserWorkItem(WorkThread, i); // Передаем идентификатор потока
        }

        Console.WriteLine("Главный поток ожидает завершения потоков...");

        // Ожидание завершения работы потоков
        Thread.Sleep(ThreadLifeTime * 1000); // Ожидаем, пока все потоки закончат свою работу
