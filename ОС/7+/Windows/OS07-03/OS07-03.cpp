...
HANDLE print_timer = CreateWaitableTimer(NULL, FALSE, L"os07-03-print");
HANDLE close_timer = CreateWaitableTimer(NULL, FALSE, L"os07-03-close");
...
int main() {
	...
	for (i = 0; iterate; i++) {
		...
		if (print) {
			cout << "Время: " << current_time << "\tИтераций: " << i << '\n';
			print = false;
		}
	}
	cout << "Итого: " << i << '\n';
...
HANDLE createThread(LPTHREAD_START_ROUTINE func) {
	...
	if (thread == NULL) {
		throw "Ошибка при создании дочернего потока";
	}
	return thread;
}
