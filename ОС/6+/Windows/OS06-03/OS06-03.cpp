...
int main() {
	...
	processes[0] = createProcess(L"D:\\BSTU\\5 sem\\ОС\\Лабы\\6\\Windows\\x64\\Debug\\OS06-03A.exe").hProcess;
	processes[1] = createProcess(L"D:\\BSTU\\5 sem\\ОС\\Лабы\\6\\Windows\\x64\\Debug\\OS06-03B.exe").hProcess;
	mutex = CreateMutex(NULL, FALSE, L"OS06-03");
	for (int i = 1; i <= 90; ++i) {
		if (i == 30)
			WaitForSingleObject(mutex, INFINITE);
		else if (i == 60)
			ReleaseMutex(mutex);
		printf("[OS06-03]\t %d.  PID = %d\n", i, pid);
		Sleep(100);
	}
